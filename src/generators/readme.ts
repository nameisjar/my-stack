...
import { getRunCommand, getInstallCommand } from '../utils/package.js';
...
`;
    ...
    await writeFile(path.join(this.config.rootPath, 'README.md'), content);
  }

  private async generateBackendReadme(): Promise<void> {
    const pm = this.config.packageManager;
    const runCmd = getRunCommand(pm);
...
`;
    ...
    await writeFile(path.join(this.config.backendPath, 'README.md'), content);
  }

  private async generateFrontendReadme(): Promise<void> {
...
`;
    ...
    await writeFile(path.join(this.config.frontendPath, 'README.md'), content);
  }
  ...
  private getFrontendFrameworkName(framework: string): string {
    const names: Record<string, string> = {
      vue: 'Vue 3',
      react: 'React',
      nextjs: 'Next.js',
  ...
  private getDatabaseName(database: string): string {
    const names: Record<string, string> = {
  ...
  private getORMName(orm: string): string {
    const names: Record<string, string> = {
      prisma: 'Prisma',
      sequelize: 'Sequelize',
      mongoose: 'Mongoose',
  ...
  private getStylingName(styling: string): string {
    const names: Record<string, string> = {
  ...
  private getStateManagementName(state: string): string {
    const names: Record<string, string> = {
      pinia: 'Pinia',
      redux: 'Redux Toolkit',
      zustand: 'Zustand',
