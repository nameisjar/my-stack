...
export class FastifyGenerator extends BaseGenerator {
  ...
  private async generatePackageJson(): Promise<void> {
    ...
    if (this.config.backend.auth === 'jwt') {
      ...
      if (isTs) devDependencies['@types/bcryptjs'] = '^2.4.6';
    }
...
`;
    ...
    await writeFile(path.join(this.backendPath, 'src', 'routes', `health.${this.ext}`), healthRoute);
...
`;
    ...
    await writeFile(path.join(this.backendPath, 'src', 'routes', `index.${this.ext}`), apiRoute);
  }
...
`;
    ...
    await writeFile(path.join(this.backendPath, 'src', 'plugins', `index.${this.ext}`), content);
  }
...
`;
    ...
    await writeFile(path.join(this.backendPath, 'src', 'schemas', `health.${this.ext}`), healthSchema);
  }
...
`;
    ...
    await writeFile(path.join(this.backendPath, 'src', 'config', `index.${this.ext}`), content);
  }
